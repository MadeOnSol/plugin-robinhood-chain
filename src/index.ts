...
export const robinhoodChainPlugin: Plugin = {
  ...
  description:
    "Query Robinhood Chain (chain id 4663) trading intelligence from MadeOnSol: EVM-native KOL trades, KOL coordination and first touches, token discovery and batch lookups, beacon-verified tokenized equities (stocks/ETFs), the liquidity-removals (rug) feed, launch-bundle detection, buyer quality, deployer reputation with trajectory/history/alerts, the DEX trade tape, and smart-money wallet rankings — from a self-hosted RHC node. Also manages the RHC rule engine: copy-trade rules, market-cap price alerts (event-driven off each trade, a few seconds, not sub-second), KOL coordination alerts and first-touch subscriptions, all with per-chain quotas. Data only — rules deliver signals, they never execute a trade.",
  actions: [
    rhcKolFeedAction,
...
export type {
  ...
  RhcCandleClosedEvent,
  RhcCandleUpdateEvent,
  RhcRiskVerdict,
  RhcRiskVerdictChangedEvent,
